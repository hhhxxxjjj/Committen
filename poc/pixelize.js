...
//   downscale (smooth) → alpha-threshold → median-cut quantize → optional outline → nearest-neighbor upscale
// Input:  Canvas (any size) with transparent background
// Output: Canvas at displaySize (long-edge), pixel-art crisp
...
export function pixelize(srcCanvas, { pixelSize = 64, paletteSize = 16, outline = 0 } = {}) {
  ...
  // 1. Smooth downscale to long-edge = pixelSize
  ...
  // 2. Alpha threshold (crisp pixel-art edges)
  for (let i = 3; i < data.length; i += 4) {
    data[i] = data[i] >= ALPHA_THRESHOLD ? 255 : 0;
  }

  // 3. Color quantization (median cut, opaque pixels only)
  quantize(data, paletteSize);

  // 4. Optional outline (in low-res space)
  if (outline > 0) {
    addOutline(data, lowW, lowH, outline);
  }

  lowCtx.putImageData(img, 0, 0);

  // 5. Nearest-neighbor upscale to display size
...
// Alpha-edge detection + inward dilation, then darken edge RGB.
// Assumes alpha is already 0 or 255 (post alpha-threshold).
function addOutline(data, w, h, thickness) {
  const n = w * h;
  let edge = new Uint8Array(n);

  // Initial edges: opaque pixel touching transparent (or canvas border)
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const i = y * w + x;
  ...
  // Inward dilation for thickness > 1
